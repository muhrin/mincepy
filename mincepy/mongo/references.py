...
class ReferenceManager:
    ...
    def __init__(
        self,
        ref_collection: pymongo.collection.Collection,
    ...
    def get_snapshot_ref_graph(
        ...
        return self._get_graph(
            ids, direction=direction, max_dist=max_dist, node_factory=db.sid_from_str
        )

    def invalidate(
        self, obj_ids: Iterable[bson.ObjectId], snapshot_ids: Iterable[types.SnapshotId]
    ):
    ...
    def _ensure_current(self, collection_name: str, ids=None):
        ...
        if to_insert:
            ...
            logger.info(
                "Updated references cache for collection '%s' with %i new entries",
                collection_name,
                len(to_insert),
            )

    def _get_missing_entries(
        self, collection: pymongo.collection.Collection, ids: List = None
    ) -> Iterator:
...
def _sid(schema_entry) -> str:
    return str(schema_entry[1])


def _oid(schema_entry) -> bson.ObjectId:
    return schema_entry[1].obj_id
